...
# رفع باگ اول پایتون: انتقال کلاس ChequeSerializer به بالای کلاس‌های وابسته
class ChequeSerializer(serializers.ModelSerializer):
    class Meta:
        model = Cheque
        fields = '__all__'
        extra_kwargs = {'payment': {'required': False}, 'expense': {'required': False}}
...
class SaleSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if total_paid > total_amount:
            raise serializers.ValidationError(f"مجموع پرداخت‌ها از مبلغ کل فاکتور بیشتر است.")
    ...
    def create(self, validated_data):
        ...
        # رفع باگ شماره ۴: استفاده از select_for_update کاملاً درون بلاک تراکنش اتمیک متد create سریالایزر
        ...
        customer.last_purchase_type = 'COMBINED' if len(payments_data) > 1 else (payments_data[0].get('payment_method') if payments_data else 'REMAINING')
        customer.save()
        ...
        for ch_data in cheques_data:
            ...
            if is_endorsed:
                ...
                if existing_cheque:
                    ...
                    existing_cheque.description = f"{existing_cheque.description or ''} | خرج شده بابت فاکتور هزینه {expense.id}"
                    existing_cheque.save()
                else:
                    Cheque.objects.create(expense=expense, is_endorsed=True, **ch_data)
            else:
                ...
                Cheque.objects.create(expense=expense, is_endorsed=False, **ch_data)
        return expense

class DamageReportSerializer(serializers.ModelSerializer):
    created_by = serializers.StringRelatedField(read_only=True)
    class Meta: model = DamageReport; fields = '__all__'

class ItemExitSerializer(serializers.ModelSerializer):
    created_by = serializers.StringRelatedField(read_only=True)
    class Meta: model = ItemExit; fields = '__all__'

class TaskSerializer(serializers.ModelSerializer):
    class Meta: model = Task; fields = '__all__'; read_only_fields = ['checklist']

class ChecklistSerializer(serializers.ModelSerializer):
    tasks = TaskSerializer(many=True, read_only=True)
    created_by = serializers.StringRelatedField(read_only=True)
    class Meta: model = Checklist; fields = '__all__'